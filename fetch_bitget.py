...
def fetch_candle_data(symbol, market_type):
    ...
    if response.status_code == 200:
        data = response.json()
        return np.array([float(candle[4]) for candle in data["data"]])  # Close prices
    else:
